...
<form class="js_ajax-forms" data-url="posts/share.php?do=publish&post_id={$post['post_id']}">
  <div class="modal-body">
    ...
    {if $post['embedded_video_link']}
      <div class="h6 text-center">
        {__("Embed Code")}
      </div>
      <div style="margin: 25px auto;">
        <textarea readonly class="form-control rounded-3 p-3 mb10" rows="4" dir="ltr" style="resize: none; font-family: monospace; font-size: 12px; line-height: 1.6; background-color: var(--bs-light, #f8f9fa);">{$post['embedded_video_iframe']}</textarea>
        <div class="text-end">
          <button type="button" class="btn btn-sm btn-light js_clipboard" data-clipboard-text="{$post['embedded_video_iframe']|escape:'html'}" data-bs-toggle="tooltip" title='{__("Copy")}'>
            <i class="fas fa-copy mr5"></i>{__("Copy")}
          </button>
        </div>
      </div>
    {/if}
