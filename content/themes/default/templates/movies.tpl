        ...
        <div class="blog-wrapper" style="border-radius: 16px;">
            ...
            {if $movie['can_watch']}
              {if $movie['source_type'] == "youtube"}
                {if $system['smart_yt_player']}
                  <div class="youtube-player js_youtube-player" data-id="{get_youtube_id($movie['source'])}">
                    <img src="https://i.ytimg.com/vi/{get_youtube_id($movie['source'])}/hqdefault.jpg">
                    <div class="play"></div>
                  </div>
                {else}
                  <div class="post-media">
                    {if $system['disable_yt_player']}
                      <div class="plyr__video-embed js_video-plyr-youtube" data-plyr-provider="youtube" data-plyr-embed-id="{get_youtube_id($movie['source'])}"></div>
                    {else}
                      <div class="ratio ratio-16x9">
                        <iframe width="560" height="315" src="https://www.youtube.com/embed/{get_youtube_id($movie['source'])}" frameborder="0" allow="autoplay; fullscreen" allowfullscreen referrerpolicy="strict-origin-when-cross-origin"></iframe>
                      </div>
                    {/if}
                  </div>
                {/if}
          ...
          {if {$movie['stars']}}
            <div>
              ...
              {foreach split($movie['stars'], ',') as $_star}
