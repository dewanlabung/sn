...
<div class="card-header with-icon with-nav">
  ...
  <ul class="nav nav-tabs">
    ...
    <li class="nav-item">
      <a class="nav-link" href="#Captcha" data-bs-toggle="tab">
        <i class="fa fa-robot fa-fw mr5"></i><strong class="pr5">{__("Captcha")}</strong>
      </a>
    </li>
  </ul>
  <!-- panel nav -->
...
<div class="tab-content">
      ...
      <div class="card-body">
        ...
        <div class="form-table-row">
          ...
          <div>
            ...
            <div class="form-text d-none d-sm-block">{__("Enable/Disable HTML code in Rich Text Editor")}</div>
          </div>
          <div class="text-end">
  ...
  <div class="tab-pane" id="Captcha">
    <form class="js_ajax-forms" data-url="admin/settings.php?edit=captcha">
      <div class="card-body">
        <div class="form-table-row">
          <div class="avatar">
          ...
          <div>
            <div class="form-label h6">{__("Captcha Enabled")}</div>
            <div class="form-text d-none d-sm-block">{__("Enable/Disable Captcha protection on registration and login forms")}</div>
          </div>
          <div class="text-end">
            <label class="switch" for="captcha_enabled">
              <input type="checkbox" name="captcha_enabled" id="captcha_enabled" {if $system['captcha_enabled']}checked{/if}>
              <span class="slider round"></span>
            </label>
          </div>
        </div>
        ...
        <div class="row form-group">
          ...
          <div class="col-md-9">
            <div>
              <!-- reCAPTCHA -->
              <input class="x-hidden input-label" type="radio" name="captcha_provider" value="recaptcha" id="captcha_recaptcha" {if $system['captcha_provider'] == "recaptcha"}checked{/if} />
              <label class="button-label" for="captcha_recaptcha">
                <div class="icon">
                  {include file='__svg_icons.tpl' icon="google_recaptcha" width="32px" height="32px"}
                </div>
                <div class="title">{__("reCAPTCHA")}</div>
              </label>
              <!-- reCAPTCHA -->
              <!-- Turnstile -->
              <input class="x-hidden input-label" type="radio" name="captcha_provider" value="turnstile" id="captcha_turnstile" {if $system['captcha_provider'] == "turnstile"}checked{/if} />
              <label class="button-label" for="captcha_turnstile">
                <div class="icon">
                  {include file='__svg_icons.tpl' icon="cloudflare" width="32px" height="32px"}
                </div>
                <div class="title">{__("Turnstile")}</div>
              </label>
              <!-- Turnstile -->
              <!-- hCaptcha -->
              <input class="x-hidden input-label" type="radio" name="captcha_provider" value="hcaptcha" id="captcha_hcaptcha" {if $system['captcha_provider'] == "hcaptcha"}checked{/if} />
              <label class="button-label" for="captcha_hcaptcha">
                <div class="icon">
                  {include file='__svg_icons.tpl' icon="hcaptcha" width="32px" height="32px"}
                </div>
                <div class="title">{__("hCaptcha")}</div>
              </label>
              <!-- hCaptcha -->
            </div>
            <div class="form-text">
              {__("Select your captcha provider")}
            </div>
          </div>
        </div>
        ...
        <div class="pl-md-4">
          <div class="row form-group">
            <label class="col-md-3 form-label">
              {__("hCaptcha Site Key")}
            </label>
            <div class="col-md-9">
              {if !$user->_data['user_demo']}
                <input type="text" class="form-control" name="hcaptcha_site_key" value="{$system['hcaptcha_site_key']}">
              {else}
                <input type="password" class="form-control" value="*********">
              {/if}
            </div>
          ...
          <div class="row form-group">
            <label class="col-md-3 form-label">
              {__("hCaptcha Secret Key")}
            </label>
            <div class="col-md-9">
              {if !$user->_data['user_demo']}
                <input type="text" class="form-control" name="hcaptcha_secret_key" value="{$system['hcaptcha_secret_key']}">
              {else}
                <input type="password" class="form-control" value="*********">
              {/if}
            </div>
