...
try {
  ...
  if (isset($_POST['voice_notes'])) {
    ...
    if (!is_object($_POST['voice_notes']) || !is_valid_upload_source($_POST['voice_notes']->source ?? null)) {
      _error(400);
    }
  }
  /* filter poll options */
  ...
  if (isset($_POST['reel'])) {
    ...
    if (!is_object($_POST['reel']) || !is_valid_upload_source($_POST['reel']->source ?? null)) {
      _error(400);
    }
  }
  /* filter video */
  if (isset($_POST['video'])) {
    ...
    if (!is_object($_POST['video']) || !is_valid_upload_source($_POST['video']->source ?? null)) {
      _error(400);
    }
  }
  /* filter audio */
  if (isset($_POST['audio'])) {
    ...
    if (!is_object($_POST['audio']) || !is_valid_upload_source($_POST['audio']->source ?? null)) {
      _error(400);
    }
  }
  /* filter file */
  if (isset($_POST['file'])) {
    ...
    if (!is_object($_POST['file']) || !is_valid_upload_source($_POST['file']->source ?? null)) {
      _error(400);
    }
  }
  ...
  if (!empty($_POST['paid_image']) && !is_valid_upload_source($_POST['paid_image'])) {
    _error(400);
  }
  ...
  $inputs['is_collaborative'] = ($_POST['is_collaborative'] == "true") ? '1' : '0';
  if ($inputs['is_collaborative']) {
    /* check if anonymous post */
    if ($inputs['is_anonymous']) {
      throw new ValidationException(__("You can not collaborate with anonymous post"));
    }
    ...
    $inputs['collaborative_users'] = $_POST['collaborative_users'];
    $inputs['collaborative_users_ids'] = $user->validate_collaborative_users($inputs['collaborative_users']);
    /* check if paid post */
    if ($inputs['is_paid']) {
      /* check if collaborative percent is valid */
      $inputs['collaborative_percent'] = $_POST['collaborative_percent'];
      if (!is_numeric($inputs['collaborative_percent']) || $inputs['collaborative_percent'] < 0 || $inputs['collaborative_percent'] > 100) {
