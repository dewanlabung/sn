...
if (!trait_exists('CampaignsTrait')) {
  trait CampaignsTrait {}
}
if (!trait_exists('FundingTrait')) {
  trait FundingTrait {}
}
if (!trait_exists('MonetizationTrait')) {
  trait MonetizationTrait {}
}
if (!trait_exists('PackagesTrait')) {
  trait PackagesTrait {}
}
if (!trait_exists('PaymentsTrait')) {
  trait PaymentsTrait {}
}
if (!trait_exists('WalletTrait')) {
  trait WalletTrait {}
}
...
class User
{
  ...
  public function __construct($jwt = null)
  {
    ...
    if (isset($user_id) && isset($user_token)) {
      ...
      if ($get_user->num_rows > 0) {
        ...
        /* check collaborative posts permission */
        $this->_data['can_add_collaborative_posts'] = $system['collaborative_posts_enabled'] && $this->check_module_permission('collaborative_posts_permission');
        /* check rich text posts permission */
        $this->_data['can_add_rich_text_posts'] = $system['rich_text_posts_enabled'] && $this->check_module_permission('rich_text_posts_permission');
        ...
        $this->_data['can_generate_ai_posts'] = $system['ai_posts_enabled'] && $this->check_module_permission('ai_posts_permission');
        /* check ai blogs permission */
        $this->_data['can_generate_ai_blogs'] = $system['ai_blogs_enabled'] && $this->check_module_permission('ai_blogs_permission');
        /* check ai images permission */
        $this->_data['can_generate_ai_images'] = $system['ai_images_enabled'] && $this->check_module_permission('ai_images_permission');
  ...
  /**
   * get_pages_managed_ids
   * 
   * @return array
   */
  public function get_pages_managed_ids()
  {
    global $db;
    $pages = [];
    if ($this->_logged_in) {
      $get_pages = $db->query(sprintf("SELECT pages.page_id FROM pages_admins INNER JOIN pages ON pages_admins.page_id = pages.page_id WHERE pages_admins.user_id = %s ORDER BY page_id DESC", secure($this->_data['user_id'], 'int')));
      if ($get_pages->num_rows > 0) {
        while ($page = $get_pages->fetch_assoc()) {
          $pages[] = $page['page_id'];
        }
  ...
  /** 
   * get_groups_managed_ids
   * 
   * @return array
   */
  public function get_groups_managed_ids()
  {
    global $db;
    $groups = [];
    if ($this->_logged_in) {
      $get_groups = $db->query(sprintf("SELECT groups.group_id FROM groups_admins INNER JOIN groups ON groups_admins.group_id = groups.group_id WHERE groups_admins.user_id = %s ORDER BY group_id DESC", secure($this->_data['user_id'], 'int')));
      if ($get_groups->num_rows > 0) {
        while ($group = $get_groups->fetch_assoc()) {
          $groups[] = $group['group_id'];
        }
  ...
  /**
   * get_events_managed_ids
   * 
   * @return array
   */
  public function get_events_managed_ids()
  {
    global $db;
    $events = [];
    if ($this->_logged_in) {
      $get_events = $db->query(sprintf("SELECT events.event_id events WHERE event_admin = %s ORDER BY event_id DESC", secure($this->_data['user_id'], 'int')));
      if ($get_events->num_rows > 0) {
        while ($event = $get_events->fetch_assoc()) {
          $events[] = $event['event_id'];
        }
  ...
  public function get_top_friends($user_id, $offset = 0, $get_all = false)
  {
    ...
    $limit_statement = ($get_all) ? "" : sprintf("LIMIT %s, %s", secure($offset, 'int', false), secure($system['max_results_even'], 'int', false));
    $get_top_friends = $db->query(sprintf("SELECT users.user_id, users.user_name, users.user_firstname, users.user_lastname, users.user_gender, users.user_picture, users.user_subscribed, users.user_verified FROM users_top_friends INNER JOIN users ON users_top_friends.friend_id = users.user_id WHERE users.user_banned = '0' AND user_incognito_enabled = '0' AND users_top_friends.user_id = %s " . $limit_statement, secure($user_id, 'int')));
  ...
  public function get_followings($user_id, $offset = 0, $get_all = false)
  {
    ...
    $limit_statement = ($get_all) ? "" : sprintf("LIMIT %s, %s", secure($offset, 'int', false), secure($system['max_results_even'], 'int', false));
    $get_followings = $db->query(sprintf("SELECT users.user_id, users.user_name, users.user_firstname, users.user_lastname, users.user_gender, users.user_picture, users.user_subscribed, users.user_verified FROM followings INNER JOIN users ON followings.following_id = users.user_id WHERE users.user_banned = '0' AND user_incognito_enabled = '0' AND followings.user_id = %s " . $limit_statement, secure($user_id, 'int')));
    if ($get_followings->num_rows > 0) {
      while ($following = $get_followings->fetch_assoc()) {
  ...
  public function get_followings_count($user_id = null)
  {
    global $db;
    $user_id = (isset($user_id)) ? $user_id : $this->_data['user_id'];
    $get_followings_count = $db->query(sprintf("SELECT COUNT(*) AS count FROM followings INNER JOIN users ON followings.following_id = users.user_id WHERE users.user_banned = '0' AND user_incognito_enabled = '0' AND followings.user_id = %s", secure($user_id, 'int')));
  ...
  public function get_followers($user_id, $offset = 0, $get_all = false)
  {
    ...
    $limit_statement = ($get_all) ? "" : sprintf("LIMIT %s, %s", secure($offset, 'int', false), secure($system['max_results_even'], 'int', false));
    $get_followers = $db->query(sprintf("SELECT users.user_id, users.user_name, users.user_firstname, users.user_lastname, users.user_gender, users.user_picture, users.user_subscribed, users.user_verified FROM followings INNER JOIN users ON followings.user_id = users.user_id WHERE users.user_banned = '0' AND user_incognito_enabled = '0' AND followings.following_id = %s " . $limit_statement, secure($user_id, 'int')));
    if ($get_followers->num_rows > 0) {
      while ($follower = $get_followers->fetch_assoc()) {
  ...
  public function get_followers_count($user_id = null)
  {
    global $db;
    $user_id = (isset($user_id)) ? $user_id : $this->_data['user_id'];
    $get_followers_count = $db->query(sprintf("SELECT COUNT(*) AS count FROM followings INNER JOIN users ON followings.user_id = users.user_id WHERE users.user_banned = '0' AND user_incognito_enabled = '0' AND followings.following_id = %s", secure($user_id, 'int')));
  ...
  public function get_blocked($offset = 0)
  {
    ...
    if ($get_blocks->num_rows > 0) {
      while ($block = $get_blocks->fetch_assoc()) {
        $block['user_fullname'] = ($system['show_usernames_enabled']) ? $block['user_name'] : $block['user_firstname'] . " " . $block['user_lastname'];
        $block['user_picture'] = get_picture($block['user_picture'], $block['user_gender']);
        $block['connection'] = 'blocked';
        $blocks[] = $block;
      }
    }
    $has_more = (count($blocks) == $system['max_results']) ? true : false;
    return ['data' => $blocks, 'has_more' => $has_more];
  }
  ...
  public function get_new_people($offset = 0, $random = false)
  {
    ...
    $where .= sprintf("WHERE users.user_id != '1' AND users.user_banned = '0' AND user_incognito_enabled = '0' AND users.user_suggestions_hidden = '0' AND users.user_id != %s AND users.user_id NOT IN (%s)", secure($this->_data['user_id'], 'int'), $this->spread_ids($old_people_ids));
  ...
  public function delete_user($user_id)
  {
    ...
    if ($can_delete) {
      ...
      /* delete all user notifications to the user */
      $db->query(sprintf('DELETE FROM notifications WHERE to_user_id = %s', secure($user_id, 'int')));
      /* delete all user notifications from the user */
      $db->query(sprintf('DELETE FROM notifications WHERE from_user_id = %s AND from_user_type = "user"', secure($user_id, 'int')));
    }
  }
  ...
  public function settings($edit, $args)
  {
    ...
    switch ($edit) {
      ...
      case 'privacy':
        ...
        /* check if incognito mode enabled */
        $args['user_incognito_enabled'] = (isset($args['user_incognito_enabled'])) ? '1' : '0';
        /* check if chat enabled */
        ...
        /* check if collaborative posts enabled */
        $args['user_privacy_collaborative'] = (isset($args['user_privacy_collaborative'])) ? $args['user_privacy_collaborative'] : 'public';
        ...
        if (!in_array($args['user_privacy_chat'], $privacy) || !in_array($args['user_privacy_poke'], $privacy) || !in_array($args['user_privacy_gifts'], $privacy) || !in_array($args['user_privacy_wall'], $privacy) || !in_array($args['user_privacy_collaborative'], $privacy) || !in_array($args['user_privacy_gender'], $privacy) || !in_array($args['user_privacy_relationship'], $privacy) || !in_array($args['user_privacy_birthdate'], $privacy) || !in_array($args['user_privacy_basic'], $privacy) || !in_array($args['user_privacy_work'], $privacy) || !in_array($args['user_privacy_location'], $privacy) || !in_array($args['user_privacy_education'], $privacy) || !in_array($args['user_privacy_other'], $privacy) || !in_array($args['user_privacy_friends'], $privacy) || !in_array($args['user_privacy_followers'], $privacy) || !in_array($args['user_privacy_subscriptions'], $privacy) || !in_array($args['user_privacy_photos'], $privacy) || !in_array($args['user_privacy_pages'], $privacy) || !in_array($args['user_privacy_groups'], $privacy) || !in_array($args['user_privacy_events'], $privacy)) {
          _error(400);
        }
        /* update user */
        $db->query(sprintf(
          "UPDATE users SET 
                    ...
                    user_incognito_enabled = %s,
                    user_privacy_chat = %s,
                    user_privacy_poke = %s,
                    user_privacy_gifts = %s,
                    user_privacy_wall = %s,
                    user_privacy_collaborative = %s,
      ...
      case 'monetization':
        ...
        $args['user_monetization_discount_enabled'] = (isset($args['user_monetization_discount_enabled'])) ? '1' : '0';
        /* validate discount percent */
        if (!is_numeric($args['user_monetization_discount_percent']) || $args['user_monetization_discount_percent'] < 1 || $args['user_monetization_discount_percent'] > 99) {
          throw new Exception(__("Please enter a valid discount percent (min 1, max 99)"));
        }
        ...
        $db->query(sprintf(
          "UPDATE users SET 
            ...
            user_monetization_discount_enabled = %s,
            user_monetization_discount_percent = %s,
            user_monetization_chat_price = %s, 
            user_monetization_call_price = %s 
           WHERE user_id = %s",
          secure($args['user_monetization_enabled']),
          secure($args['user_monetization_discount_enabled']),
          secure($args['user_monetization_discount_percent'], 'float'),
          secure($args['user_monetization_chat_price'], 'float'),
          secure($args['user_monetization_call_price'], 'float'),
          secure($this->_data['user_id'], 'int')
        ));
  ...
  public function sign_up($args = [], $device_info = [])
  {
    ...
    /* check reCAPTCHA */
    if ($system['captcha_enabled'] && $system['captcha_provider'] == "recaptcha" && $args['from_web']) {
    ...
    /* check Turnstile */
    if ($system['captcha_enabled'] && $system['captcha_provider'] == "turnstile" && $args['from_web']) {
      if (!check_cf_turnstile($args['cf-turnstile-response'])) {
        throw new ValidationException(__("The security check is incorrect. Please try again"));
      }
    }
    /* check hCaptcha */
    if ($system['captcha_enabled'] && $system['captcha_provider'] == "hcaptcha" && $args['from_web']) {
      if (!check_hcaptcha($args['h-captcha-response'])) {
        throw new ValidationException(__("The security check is incorrect. Please try again"));
      }
    }
  ...
  public function social_signin($provider, $user_profile)
  {
    ...
    switch ($provider) {
      case 'apple':
        $social_id = "apple_id";
        $social_connected = "apple_connected";
        break;
      ...
      case 'discord':
        ...
        $social_connected = "discord_connected";
        break;
   ...
   * @param string $hcaptcha_response
   * @param bool $from_web
   * @return void
   */
  public function forget_password($email, $recaptcha_response = null, $turnstile_response = null, $hcaptcha_response = null, $from_web = false)
  {
    global $db, $system;
    ...
    /* check reCAPTCHA */
    if ($system['captcha_enabled'] && $system['captcha_provider'] == "recaptcha" && $from_web) {
      $recaptcha = new \ReCaptcha\ReCaptcha($system['reCAPTCHA_secret_key'], new \ReCaptcha\RequestMethod\CurlPost());
    ...
    /* check Turnstile */
    if ($system['captcha_enabled'] && $system['captcha_provider'] == "turnstile" && $from_web) {
      if (!check_cf_turnstile($turnstile_response)) {
        throw new ValidationException(__("The security check is incorrect. Please try again"));
      }
    }
    /* check hCaptcha */
    if ($system['captcha_enabled'] && $system['captcha_provider'] == "hcaptcha" && $from_web) {
      if (!check_hcaptcha($hcaptcha_response)) {
        throw new ValidationException(__("The security check is incorrect. Please try again"));
      }
    }
    /* generate reset key */
