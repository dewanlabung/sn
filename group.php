...
try {
  ...
  switch ($_GET['view']) {
    ...
    case 'blogs':
      ...
      if (!$group['needs_subscription']) {
        /* get posts (blogs) */
        $posts = $user->get_posts(['get' => 'posts_group', 'id' => $group['group_id'], 'filter' => 'article']);
        /* assign variables */
        $smarty->assign('posts', $posts);
      }
      break;
    ...
    case 'settings':
      ...
      switch ($sub_view) {
        ...
        case 'monetization':
          ...
          /* get sub view content */
          switch ($_GET['show']) {
            case '':
            case 'settings':
              /* set the show if not set */
              if (!isset($_GET['show'])) {
                $_GET['show'] = 'settings';
              }

              /* get monetozaion plans */
              $smarty->assign('monetization_plans', method_exists($user, 'get_monetization_plans') ? $user->get_monetization_plans($group['group_id'], 'group') : []);
            ...
            case 'coupons':
              /* get coupons */
              $smarty->assign('coupons', method_exists($user, 'get_monetization_coupons') ? $user->get_monetization_coupons($group['group_id'], 'group') : []);
              break;
